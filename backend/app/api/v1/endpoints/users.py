...
async def _require_workspace_admin(
    ...
    """Admin check for dev: the first registered auth user in a workspace is the admin."""
    ...
    res = await db.execute(
        select(AuthUser).where(AuthUser.workspace_id == workspace_slug).order_by(AuthUser.id.asc()).limit(1)
    )
    first_user = res.scalars().first()
    if not first_user or first_user.email != email:
        raise HTTPException(status_code=403, detail="Admin privileges required")

    return {"workspace_slug": workspace_slug, "email": email}
...
async def admin_create_user(
    ...
    # Create corresponding workspace user record (used elsewhere in the app).
    ...
    try:
        ...
        email_debug = {"error": str(e), "to": payload.email, "subject": subject, "text_body": text_body}
        email_sent = False
