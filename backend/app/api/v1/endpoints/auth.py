...
async def register(payload: RegisterIn, db: AsyncSession = Depends(get_db)) -> TokenOut:
    ...
    tenant_key = project_name.lower().strip().replace(" ", "-")

    res = await db.execute(select(Tenant).where(Tenant.tenant_id == tenant_key))
    project = res.scalars().first()
    if not project:
        project = Tenant(name=project_name)
        project.tenant_id = tenant_key
        db.add(project)
        await db.flush()
...
async def login(payload: LoginIn, db: AsyncSession = Depends(get_db)) -> TokenOut:
    res = await db.execute(select(Tenant).where(Tenant.id == payload.project_id))
    project = res.scalars().first()
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")

    res = await db.execute(
        select(AuthUser).where(AuthUser.tenant_id == project.tenant_id, AuthUser.email == payload.email)
    )
    user = res.scalars().first()
    if not user or not verify_password(payload.password, user.password_hash):
        raise HTTPException(status_code=401, detail="Invalid email or password")

    token = create_access_token(project_id=project.id, tenant_id=project.tenant_id, email=payload.email)
    return TokenOut(access_token=token)
